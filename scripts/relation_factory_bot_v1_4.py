...
def app_copy(drug, counterpart, ctype, action):
    if action == "separation":
        disp = f"이 약은 {counterpart}과(와) 같은 시간에 복용하면 이 약의 흡수가 줄어들 수 있다는 허가사항 문구가 있습니다. 자세한 사항은 약사 또는 의사와 상담하세요."
        mng = f"같은 시간대 복용은 피하고 시간 간격을 두는 것이 도움이 될 수 있습니다. 구체적인 간격은 약사 또는 의사와 상담하세요."
    else:  # monitoring/depletion
        disp = f"이 약을 장기간 복용할 때 {counterpart}과(와) 관련된 허가사항 주의 문구가 있습니다. 증상이 걱정되면 약사 또는 의사와 상담하세요."
        mng = f"장기 복용 중이라면 정기 진료나 복약 상담 시 해당 영양소 상태 확인이 필요한지 문의해볼 수 있습니다."
    return disp, mng
