...
def main(json_path, csv_path, alias_path, rel_path, ar_path=DEF_AR, ar2_path=DEF_AR2, ar3_path=DEF_AR3, ar4_path=DEF_AR4, ar5_path=DEF_AR5, ar6_path=DEF_AR6, ar7_path=DEF_AR7, ar8_path=DEF_AR8, ar9_path=DEF_AR9):
    ...
    # --- approved-ready batch6 검증(있을 때, Phase 13/v0.6, 번호 120~135): 미반영(incorporated=false) 전제 + ≤50 ---
    if os.path.exists(ar6_path):
        _validate_approved_ready(v, ar6_path, cands, existing, allowed, excluded_only, alias_seqs, wl_by_canon,
                                 base_no=120, tag="batch6")
        ar6_data, _e6 = load_json(ar6_path, "approved-ready batch6")
        ar6 = ar6_data.get("approved_ready") if isinstance(ar6_data, dict) else None
        if isinstance(ar6, list):
            v.check(len(ar6) <= 50, 133, "batch6 approved-ready ≤ 50건(v0.6 +50 전략)", f"count={len(ar6)}")
            # (Phase 14 옵션 A) 반영 전=false / 반영 후=true 둘 다 정합. incorporated=true 의 실제 반영 검증은 base+12(#132).
            bad_inc6 = [e.get("candidate_alias") for e in ar6 if str(e.get("incorporated", "")).strip().lower() not in ("false", "true")]
            v.check(not bad_inc6, 134, "batch6 approved-ready incorporated ∈ {false(미반영),true(반영)} 정합(true는 #132에서 실제 반영 검증)", f"viol={bad_inc6}")
            no_inc6 = [e.get("candidate_alias") for e in ar6 if "incorporated" not in e]
            v.check(not no_inc6, 135, "batch6 approved-ready 는 incorporated 필드 보유", f"viol={no_inc6}")

    # --- approved-ready batch7 검증(있을 때, Phase 15/v0.6, 번호 140~155): 미반영(incorporated=false) 전제 + ≤50 ---
    if os.path.exists(ar7_path):
        _validate_approved_ready(v, ar7_path, cands, existing, allowed, excluded_only, alias_seqs, wl_by_canon,
                                 base_no=140, tag="batch7")
        ar7_data, _e7 = load_json(ar7_path, "approved-ready batch7")
        ar7 = ar7_data.get("approved_ready") if isinstance(ar7_data, dict) else None
        if isinstance(ar7, list):
            v.check(len(ar7) <= 50, 153, "batch7 approved-ready ≤ 50건(v0.6 +50 전략)", f"count={len(ar7)}")
            # (Phase 16 옵션 A) 반영 전=false / 반영 후=true 둘 다 정합. incorporated=true 의 실제 반영 검증은 base+12(#152).
            bad_inc7 = [e.get("candidate_alias") for e in ar7 if str(e.get("incorporated", "")).strip().lower() not in ("false", "true")]
            v.check(not bad_inc7, 154, "batch7 approved-ready incorporated ∈ {false(미반영),true(반영)} 정합(true는 #152에서 실제 반영 검증)", f"viol={bad_inc7}")
            no_inc7 = [e.get("candidate_alias") for e in ar7 if "incorporated" not in e]
            v.check(not no_inc7, 155, "batch7 approved-ready 는 incorporated 필드 보유", f"viol={no_inc7}")

    # --- approved-ready batch8 검증(있을 때, Phase 17/v0.6, 번호 160~175): 미반영(incorporated=false) 전제 + ≤50 ---
    if os.path.exists(ar8_path):
        _validate_approved_ready(v, ar8_path, cands, existing, allowed, excluded_only, alias_seqs, wl_by_canon,
                                 base_no=160, tag="batch8")
        ar8_data, _e8 = load_json(ar8_path, "approved-ready batch8")
    ...
    if os.path.exists(ar9_path):
        ...
        if isinstance(ar9, list):
            ...
            # (Phase 20 옵션 A) 반영 전=false / 반영 후=true 둘 다 정합. incorporated=true 의 실제 반영 검증은 base+12(#192).
            bad_inc9 = [e.get("candidate_alias") for e in ar9 if str(e.get("incorporated", "")).strip().lower() not in ("false", "true")]
            v.check(not bad_inc9, 194, "batch9 approved-ready incorporated ∈ {false(미반영),true(반영)} 정합(true는 #192에서 실제 반영 검증)", f"viol={bad_inc9}")
