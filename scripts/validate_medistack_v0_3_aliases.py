...
#       '비타민D' 명시로 칼슘 카드 오인 차단 — 안전게이트 통과).
# v1.1 buffer_combo(C): 란소프라졸 개방(PPI+침강탄산칼슘 18건. 칼슘=위산중화 완충제(영양칼슘 아님)·신규 relation 0·
#       표시카드=PPI×B12/×Mg(id36/37·32/33)·other_label="위산 중화 완충 성분(침강탄산칼슘)"로 칼슘 오독 차단).
#       라베프라졸은 B/D 로 기개방. 라베+산화Mg(E)는 계속 미개방(직접모순). 에스오메프라졸 영구 하드차단.
COMBO_ALLOWED_BASIS = {"메트포르민", "알렌드론산", "오메프라졸", "히드로클로로티아지드",
                       "라베프라졸", "리세드론산", "이반드론산", "란소프라졸"}
