#!/usr/bin/env python3
"""
integrate_coverage_queue_draft_batch_v1_2.py
MediStack — coverage-queue draft batch 중 **CQF01(알마게이트×철분)만** 라이브 통합한다.
PM 승인(이번 라운드 프롬프트) = CQF01 do_not_implement_yet 해제. batch2 다른 행은 없음(count=1).

수행(integrate_relation_draft_v1_2.py flip+alias 패턴 + integrate_factory_draft_batch_v1_2.py STOP 가드 승계):
  1) v0.2 export(라이브 DATA_URL): relations 57 → 58 (CQF01 append, id 59). meta.relation_count 58.
  ...
  2) full index: 알마게이트 **단일성분** name_only 품목(54건)만 relation_card 로 flip.
     복합제(알마게이트/유당수화물 1건)는 보수적 name_only 유지(v1.1/v1.2 복합제 패턴 승계).
     meta.counts 갱신(relation_card 1077→1131·name_only 16503→16449·total 17580 유지).
  3) aliases: verified_item_seqs 에 알마게이트 키 추가(flip 된 item_seq, pool 진입).
     alias_count 717 불변(verified_item_seqs 는 alias_count 미포함).

⚠️ 절대 가드:
  - CQF01 외 통합 금지. 칼륨(potassium_safety_card)·depletion·금지성분 들어오면 STOP.
  - mechanism=absorption·source itemSeq 보유·draft adversarial_verified=true·source_confirmed=true 여야 함.
  - 기존 relation 57·excluded·disclaimers·DATA_URL·src·published/clinical false 불변.
  - reviewed_by 등 reviewer 크레딧 부여 금지(clinical_reviewed=false 유지·reviewer 미기재).
idempotent: 알마게이트×철분 relation 이 이미 export 에 있으면 export/full/alias 각각 skip.
사용: python3 scripts/integrate_coverage_queue_draft_batch_v1_2.py [--dry-run]
"""
import json
import os
import re
...
BASELINE_RELATIONS = 57
TARGET_DRAFT_IDS = ["CQF01"]          # 이번 승격 대상(알마게이트×철분)만
FLIP_INGREDIENT = "알마게이트"         # 신규 relation 성분(full index 단일성분 flip 대상)
...
def is_flip_candidate(e):
    ...
    if "/" in ing or "," in ing:        # 단일성분만(복합 보수적 name_only 유지)
        return False
    return ing == FLIP_INGREDIENT
...
def main():
    ...
    if not already:
        if len(exp["relations"]) != BASELINE_RELATIONS:
            ...
            return 1
        if len(targets) != 1:
            print(f"[STOP] 대상 draft {len(targets)} != 1 (CQF01)")
            return 1
        for d in targets:
            tag = d["draft_id"]
    ...
    # ---- 1) export append ----
    if already:
        print("[skip] export 이미 통합(알마게이트×철분 relation 존재)")
        ...
        exp["meta"]["note"] = exp["meta"].get("note", "") + \
            (" | coverage-queue draft batch CQF01 통합(2026-06-14): 알마게이트(제산제 단일제)×철분 "
             "absorption/separation 1건(id 59, 허가사항 상호작용 출처·적대적 검증 통과). relation 57→58. "
             "알마게이트 단일성분 54품목 full index relation_card flip.")
    ...
    print(f"[full index] flip {len(flip)}건(알마게이트 단일성분) → "
    ...
    print("INTEGRATE COVERAGE-QUEUE DRAFT BATCH (CQF01): DONE")
    return 0
