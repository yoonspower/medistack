...
### 19.7 최종 체크리스트 보강 — 칼륨 트랙과의 통합 순서·핸드오프(2026-06-15)
AT-FEX 와 **칼륨 PM-ready 3건**(DF01·DF04·DF05, `docs/MediStack_clinical_reviewer_handoff_v1_2.md` §1-B)은 **서로 독립된 reviewer 판정**이다 — 한 묶음으로 승격하거나 한쪽 노트로 다른 쪽을 승격하지 말 것. 다만 둘 다 같은 v0.2 export 의 relation-count 에 영향을 주므로 **통합 순서**에 주의한다:
- **id 충돌 없음**: 두 integrate 스크립트(`integrate_antacid_fex_v1_2.py`·`integrate_potassium_pm_ready_v1_2.py`)는 모두 `id = max(id)+1` 을 **런타임 계산**한다. 먼저 통합되는 쪽이 다음 baseline 을 올리므로 id 는 자동 정합(예: AT-FEX 먼저면 62, 그다음 칼륨 63~65 / 칼륨 먼저면 62~64, 그다음 AT-FEX 65).
- **하드코딩 count validator 는 누적 갱신**: relation-count 를 상수로 박은 검증기들은 **실제 통합 건수만큼 누적**으로 올려야 한다(AT-FEX +1, 칼륨 +3). 두 트랙을 같은 시기 통합하면 60→64(둘 다) 또는 부분합. 통합 전 `git grep -nE '\b(59|60|61)\b' scripts/validate_*` 로 잔존 하드코딩을 점검하고, 통합 직후 전수 PASS 를 재확인한다.
- **상호 비간섭**: AT-FEX 의 #15(avoid_concomitant·antacid 가드)와 칼륨의 #11(칼륨⇒link=false&card=true)은 서로 다른 relation 집합에만 걸려 충돌하지 않는다(antacid 는 counterpart_category=al_mg_antacid, 칼륨은 nutrient=칼륨). 한쪽 통합이 다른 쪽 검증을 깨지 않음(드라이런으로 각각 시뮬 v0.2 PASS 확인).
- **공통 전제**: 양쪽 모두 `--pm-approved` (+ 칼륨은 `--reviewer-note`) 없이는 write 0. clinical reviewer 노트 전까지 둘 다 `live_integration_forbidden=true` 유지.
- **reviewer-note 인터록(칼륨이 참조 구현)**: `integrate_potassium_pm_ready_v1_2.py` 의 노트 게이트는 **의미적**이다 — 노트가 비공란 + 승인 토큰('approved'|'승인') + **승격 대상 draft_id 전건 명시**여야 통과(garbage/공백/일부 누락 거부, 적대검증으로 입증). **AT-FEX 통합 시 같은 인터록을 `integrate_antacid_fex_v1_2.py` 에 보강할 것**(현재는 `--pm-approved` 단일 게이트 — reviewer-note 인자 미보유).
