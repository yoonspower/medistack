...
> 🎯 **커버리지 우선 게이트(2026-06-14 지시):** 현재 최우선은 **relation coverage 확장**(relation ≥1,000 · relation_card ≥10,000 · 검색 UX 안정화)이며, 그 전까지 **유료화/결제/구독/계정/동기화 구현은 보류**다. Supabase 의 진짜 동기(§1)인 saved_stack 기기 간 동기화·`plus_entitlements`(§2.2)는 **유료화 트랙에 묶여 있으므로 본 게이트가 풀리기 전까지 착수하지 않는다.** 따라서 §4 의 5단계(user feature Supabase)는 **"coverage 목표 달성 + 유료화 승인" 이후**로 명시 이연한다. 그때까지 본 문서는 **설계·조건·순서 동결 상태**로 유지한다(코드/DB/배선 0). 정적 GitHub JSON + Pages 로 relation 1,000·relation_card 10,000 까지는 **충분히 스케일**(읽기전용·전역공통·저빈도 갱신 데이터, §1) — 즉 coverage 확장 자체가 Supabase 도입 사유가 되지 않는다.
