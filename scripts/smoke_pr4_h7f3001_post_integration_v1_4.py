#!/usr/bin/env python3
"""
smoke_pr4_h7f3001_post_integration_v1_4.py
PR-4 H7-F3-001 통합 후 **앱 렌더/검색 스모크** (읽기전용·live 무수정).
  - disclaimers.common 존재
  - 신규 1건 display_text_ko/management_ko/source 존재 · potassium_safety_card=false
  - 신규 al_mg_antacid(약물 counterpart) → nutrient 표기에 '약물' 포함 · counterpart_category=al_mg_antacid
  - display 라벨귀속 '분리하도록 안내' 단정 비노출 · '효과가 감소' 비노출
  - 검색 스모크: 리세드론산/제산제
"""
import json
import os
...
def main():
    ...
    bad_eff = [r["id"] for r in new_rels if "효과가 감소" in f"{r.get('display_text_ko','')} {r.get('management_ko','')}"]
    ok("신규 display/management '효과가 감소' 비노출", not bad_eff, str(bad_eff))

    for tok, lo in [("리세드론산", 1), ("제산제", 1)]:
    ...
    if _fail:
        ...
        return 1
    print("RESULT: PASS — 신규 1건 렌더/검색 계약 OK")
    return 0
