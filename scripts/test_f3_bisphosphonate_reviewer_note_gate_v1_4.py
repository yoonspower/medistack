#!/usr/bin/env python3
"""
test_f3_bisphosphonate_reviewer_note_gate_v1_4.py
MediStack — F3 비스포스포네이트 live 통합 reviewer-note **인터록 회귀 테스트**(읽기전용·네트워크 0).

검증 대상: integrate_f3_bisphosphonate_batch_v1_4.check_reviewer_note + (temp-copy) 전체 write 경로 + needs_review 통합 차단.
  1) 게이트 단위 — invalid(빈/토큰없음/candidate 누락/scope 불일치/SAMPLE/placeholder/clinical=true 요구/제품 추천 허용/
     금속이온·제산제·우유 복용 권유 허용/al_mg_antacid 미명시/이반드론산 overlap 누락/pediatric·계열 일반화·에티드론산 standalone 일반화 허용/
     verified_reference 없음/reviewer 식별자 없음/grouping·간격 누락) 거부 + valid 통과.
  2) temp-copy 전체 write — valid 노트로 survives scope(0147) 60→61(id 62) 동작 + **실제 live export sha256 불변**.
  3) needs_review 차단 — --candidate-ids 로 0148(needs_review) 요청 시 build_subset STOP(통합 거부).
...
def build_valid(scope_label, scope_ids):
    ...
    grouping = {"survives": "grouping: survives subset 한 번에 통합",
                "antacid1": "grouping: by-counterpart al_mg_antacid subset"}[scope_label]
    return (
        f"검수자: RPH-TEST-001 (PM 승인 근거 첨부)  검토일 2026-07-01\n"
        f"승인(approved): F3 비스포스포네이트 {scope_label} 후보를 verified_reference 노출로 승인.\n"
        ...
        f"이반드론산 nutrient-overlap 판단: 기존 ×칼슘/철분/마그네슘 영양소 relation 과 정보 중복 아닌 "
        f"제산제 제품 맥락으로 추가 노출 승인.\n"
        f"separation 간격(예: 30분/2시간) 카드 노출: 일반 '분리' 안내 유지(구체 시간 비노출).\n"
        f"clinical_reviewed=true 아님(verified_reference 천장 유지). 제품·구매·제휴 추천 없음. "
        f"금속이온·제산제·우유·유제품 복용 권유 없음.\n"
        f"에티드론산 0148/0149 standalone 칼슘/철분 parse 는 본 승인 대상 아님(needs_review 유지).\n"
    )


def test_gate(mod, tmp):
    print("--- F3 게이트 (check_reviewer_note) ---")
    recs, _s = mod.load_f3()
    survives = mod._survives_ids(recs)
    VALID = build_valid("survives", survives)
    cn = lambda txt, ids=survives: mod.check_reviewer_note(_note(tmp, txt), ids)

    expect_reject("빈 노트 거부", cn("")[1], "비공란")
    expect_reject("승인 토큰 없음 거부",
                  cn(VALID.replace("승인", "검토").replace("approved", "review"))[1], "승인 표기")
    expect_reject("candidate_id 누락 거부", cn(VALID.replace("RF-F3-0147", "RF-F3-XXXX"))[1], "candidate_id 미명시")
    ...
    expect_reject("grouping 결정 누락 거부",
                  cn(VALID.replace("grouping: survives subset 한 번에 통합", "통합 방식 미정"))[1], "grouping")
    expect_reject("al_mg_antacid 미명시 거부",
                  cn(VALID.replace("al_mg_antacid(약물 counterpart·id61 선례) — 마그네슘 영양제 아님",
                                   "제산제 분류 미정"))[1], "al_mg_antacid")
    expect_reject("이반드론산 nutrient-overlap 판단 누락 거부",
                  cn(VALID.replace("이반드론산 nutrient-overlap 판단: 기존 ×칼슘/철분/마그네슘 영양소 relation 과 정보 중복 아닌 "
                                   "제산제 제품 맥락으로 추가 노출 승인.\n", ""))[1], "overlap")
    expect_reject("separation 간격 결정 누락 거부",
                  cn(VALID.replace("separation 간격(예: 30분/2시간) 카드 노출: 일반 '분리' 안내 유지(구체 시간 비노출).\n", ""))[1],
                  "간격")
    ...
    expect_reject("금속이온/제산제/유제품 복용 권유 허용 거부",
                  cn(VALID.replace("금속이온·제산제·우유·유제품 복용 권유 없음.", "칼슘 보충 권장함."))[1], "복용 권유/권장 허용")
    expect_reject("pediatric/계열 일반화 허용 거부",
                  cn(VALID + "\n소아·골형성 문맥 비스포스포네이트 계열 일반화 승인.")[1], "일반화")
    expect_reject("에티드론산 standalone 일반화 허용 거부",
                  cn(VALID.replace("에티드론산 0148/0149 standalone 칼슘/철분 parse 는 본 승인 대상 아님(needs_review 유지).",
                                   "에티드론산 standalone 칼슘 일반화 승인함."))[1], "일반화")
    expect_accept("valid 노트(survives) 통과", cn(VALID)[1])
    # scope 불일치 — 다른 후보 id 로 검증(scope_ids 에 없는 id 요구 시 candidate 미명시)
    expect_reject("scope 불일치(0148 추가 요구) 거부",
                  mod.check_reviewer_note(_note(tmp, VALID), survives + ["RF-F3-0148"])[1], "candidate_id 미명시")
...
def temp_write(mod, tmp, scope_label, expected_after, expected_ids):
    ...
    recs, _s = mod.load_f3()
    survives = mod._survives_ids(recs)
    scope_ids = {"survives": survives, "antacid1": survives}[scope_label]
    work = tempfile.mkdtemp(prefix="ms_f3_write_")
    old_argv = sys.argv
    try:
        ...
        argv = ["prog", "--pm-approved", "--reviewer-note", note, "--scope", scope_label]
        sys.argv = argv
        rc = mod.main()
        ...
        for c, n in ((ok_rc, "rc=0"), (ok_cnt, f"temp relations {expected_after}"),
                     ...
                     (ok_cat, "제산제만 al_mg_antacid category")):
            ok = bool(c)
...
def test_needs_review_block(mod, tmp):
    print("--- needs_review 통합 차단 (--candidate-ids 0148 STOP) ---")
    real = mod.EXPORT
    before = _sha(real)
    survives = mod._survives_ids(mod.load_f3()[0])
    work = tempfile.mkdtemp(prefix="ms_f3_nr_")
    old_argv = sys.argv
    try:
        tmp_export = os.path.join(work, "export.json")
        shutil.copy(real, tmp_export)
        # 0147+0148 을 명시 요청 — 0148 은 needs_review → build_subset viol → STOP(노트 검사 전 차단)
        note = _note(tmp, build_valid("survives", survives + ["RF-F3-0148"]))
        mod.EXPORT = tmp_export
        sys.argv = ["prog", "--pm-approved", "--reviewer-note", note, "--candidate-ids", "RF-F3-0147,RF-F3-0148"]
        rc = mod.main()
        ok = rc == 1
        print(("  PASS " if ok else "  FAIL ") + "0148(needs_review) 통합 요청 STOP" + ("" if ok else f" rc={rc}"))
        if not ok:
            fails.append("needs_review block")
        # temp export 도 무수정(STOP 이 write 전)
...
def test_idempotency(mod, tmp):
    ...
    before = _sha(real)
    survives = mod._survives_ids(mod.load_f3()[0])
    work = tempfile.mkdtemp(prefix="ms_f3_idem_")
    old_argv = sys.argv
    try:
        tmp_export = os.path.join(work, "export.json")
        shutil.copy(real, tmp_export)
        note = _note(tmp, build_valid("survives", survives))
        mod.EXPORT = tmp_export
        sys.argv = ["prog", "--pm-approved", "--reviewer-note", note, "--scope", "survives"]
        mod.main()  # 1차 통합
        rc2 = mod.main()  # 2차 — 이미 존재 → build_subset viol → STOP(1)
        ok = rc2 == 1
        print(("  PASS " if ok else "  FAIL ") + "survives 재실행 STOP(이미 live)" + ("" if ok else f" rc2={rc2}"))
        if not ok:
...
def main():
    mod = _load("integrate_f3_bisphosphonate_batch_v1_4.py")
    tmp = tempfile.mkdtemp(prefix="ms_f3_gate_")
    try:
        print("=== F3 비스포스포네이트 reviewer-note 게이트 회귀 테스트 (live 무수정) ===")
        test_gate(mod, tmp)
        temp_write(mod, tmp, "survives", 61, [62])
    ...
    if fails:
        ...
        return 1
    print("RESULT: PASS — invalid 전건 거부 + valid 통과 · temp write survives 60→61(id 62) · "
          "needs_review(0148) 통합 차단 · scope 불일치 거부 · idempotency STOP · live export 무수정")
    return 0
