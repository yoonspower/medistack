#!/usr/bin/env python3
"""
check_pr2_chronic8_pm_note_v1_4.py
MediStack v1.4 — PR-2 **chronic8** PM-reviewed verified-reference note **checker** (읽기전용·네트워크 0·live 무수정).

PR-2 chronic8 = F9 만성복용 depletion 7 (엽산 3 + 비타민D 4) + F6 에스오메프라졸 × 비타민B12 1 = 8건.
relation_count 84 → 92.
...
(1) PM 토큰 전건 (2) candidate 8 전건 (3) 보호 상태 유지 승인 (4) 금지 행위 미요구
(5) F9/F6 copy 원칙(수치 단정 회피·골질환 alarm 비노출·모니터링 톤) 인지 임을 확인한다.

wave→candidate_ids/delta 는 data/review/pr2_chronic8_candidate_lock_v1_4.json 에서 읽는다(단일 진실원).

사용:
  python3 scripts/check_pr2_chronic8_pm_note_v1_4.py --note data/review/pr2_chronic8_pm_reviewed_note_v1_4.md
PASS=종료코드 0, FAIL=1.
"""
import argparse
import json
...
# note 본문에 등장하면 즉시 거부 — 검토자가 금지 행동을 '요구/허용/단정'.
FORBIDDEN = [
...
# clinical/published=true 승격 '요구' 또는 검수완료 '단정' (부정 '아님/아닙/없음/금지/유지' 직후는 제외)
...
# 엽산/비타민D/B12 보충·복용 권유를 '허용/권장' (부정 직후 제외) — F9/F6 영양제 복용 권유 금지
SUPPLEMENT_RECO_PERMISSION_RE = re.compile(
    r"(엽산|비타민\s*D|비타민\s*B12|B12|영양제|보충제)[ \t]*(보충|복용|섭취)?[ \t]*"
    r"(권장|권유|하세요|하십시오|드세요|섭취하|복용하|허용)(?![ \t]*(안|불가|금지|없|아님|아닙))")
# 검사/처방/투여 '지시'를 '허용/추가' — 모니터링 톤 위반(부정 마커는 permission 단어가 없어 매치 안 됨)
TEST_TREAT_PERMISSION_RE = re.compile(
    r"(검사|처방|투여)[ \t]*(지시|받으세요|하세요|권고)[ \t]*(허용|추가|노출|승인)"
    r"|(검사|처방)[ \t]*지시[ \t]*(문구[ \t]*)?(허용|추가)")

# 제외 family — PR-2 는 F9/F6 만 in-scope. F1/F2/F3/F4/F10 candidate 가 승인 맥락(제외 라인 밖)에 등장하면 거부.
EXCLUDED_FAMILY_RE = re.compile(r"\bRF-(F1|F2|F3|F4|F10)-\S+")
...
def check_pm_note(note_path, lock):
    ...
    # 0) lock 무결성 — 전 candidate has_source (source 없는 후보 승격 차단)
    ...
    if "F9" not in fams:
        problems.append("lock 에 F9 후보 없음")
    if "F6" not in fams:
        problems.append("lock 에 F6 후보 없음")
    ...
    if SUPPLEMENT_RECO_PERMISSION_RE.search(t):
        problems.append("엽산/비타민D/B12 보충·복용 권유 허용 문구 — 금지")
    if TEST_TREAT_PERMISSION_RE.search(t):
        problems.append("검사/처방/투여 지시 카피 허용 문구 — 금지(모니터링 톤)")
    ...
    if "chronic8" not in t:
        problems.append("wave(PR-2 chronic8) 명시 없음")
    if "live/pr2-chronic8" not in t and "target" not in low and "branch" not in low:
    ...
    # 6) scope 구성 명시 (F9 7 / F6 1)
    if not re.search(r"F9[^\n]*?7", t):
        problems.append("scope: F9 7 명시 없음")
    if not re.search(r"F6[^\n]*?1", t):
        problems.append("scope: F6 1 명시 없음")
    if not re.search(r"엽산[^\n]*?3", t):
        problems.append("scope: 엽산 3 명시 없음")
    if not re.search(r"비타민\s*D[^\n]*?4", t):
        problems.append("scope: 비타민D 4 명시 없음")

    # 7) needs_review / 제외 family / PR-1 재추가 금지 — '제외' 맥락에서만 허용
    nr_bad, fam_bad = [], []
    for line in t.splitlines():
        ...
        for full in EXCLUDED_FAMILY_RE.finditer(line):
            ...
            if not in_scope and "제외" not in line and "PR-1" not in line and "PR1" not in line:
                fam_bad.append(fid)
    if nr_bad:
        problems.append(f"needs_review candidate 포함(승인 맥락): {sorted(set(nr_bad))}")
    if fam_bad:
        problems.append(f"F1/F2/F3/F4/F10 candidate 포함(승인 맥락): {sorted(set(fam_bad))}")
    if "needs_review" not in t or "제외" not in t:
        problems.append("needs_review 제외 확인 문구 없음")
    if not re.search(r"F3.*F4|F3/F4", t):
        problems.append("F3/F4 family 제외(PR-3 이관) 확인 문구 없음")
    if "RF-F9-0245" not in t:
        problems.append("RF-F9-0245(카르바마제핀×엽산) needs_review 인지 명시 없음")
    if "PR-1" not in t or ("재추가" not in t and "재통합" not in t and "중복" not in t):
        problems.append("PR-1 후보 재추가 금지 확인 문구 없음")
    ...
    # 9) source fidelity / copy_change 반영
    ...
    # F9/F6 copy 원칙: 수치 단정 회피 + 골질환 alarm 비노출 인지
    if "수치" not in t or ("단정" not in t and "비노출" not in t and "보수" not in t):
        problems.append("'수치 저하 단정 회피' copy 원칙 확인 없음")
    if not re.search(r"(골연화증|골질환|골다공증|구루병)", t) or "비노출" not in t:
        problems.append("비타민D 골질환 alarm 비노출 확인 없음")

    # 10) grouping / management copy 보수성 / 모니터링 톤
    ...
    if "모니터링" not in t and "monitoring" not in low:
        problems.append("모니터링 톤(검사/처방 지시 아님) 확인 없음")
    if "장기" not in t and "연용" not in t:
        problems.append("장기/연용 복용 framing 확인 없음")
...
def main():
    ap = argparse.ArgumentParser(description="PR-2 chronic8 PM-reviewed note checker (no write)")
    ...
    if ok:
        print(f"PASS — PR-2 chronic8 PM note 유효 "
              f"(candidate {lock['total']} · delta +{lock['relation_delta']} · "
              f"{lock['baseline_relation_count']}→{lock['expected_relation_count_after']} · "
              f"needs_review/F3·F4 제외 · PM 토큰 {len(PM_TOKENS)})")
        return 0
    print(f"FAIL — PR-2 chronic8 PM note 거부 ({len(problems)}건):")
    for p in problems:
        print("  -", p)
    return 1
