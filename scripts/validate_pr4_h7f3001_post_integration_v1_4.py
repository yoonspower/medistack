#!/usr/bin/env python3
"""
validate_pr4_h7f3001_post_integration_v1_4.py
PR-4 H7-F3-001 **통합 후** v0.2 export 검증 (읽기전용·live 무수정).

검증:
  - relation_count == 95 (meta + len)
  - baseline(base commit 7f07df4) 94건이 한 건도 변경/삭제 없이 보존 (PR-1 62-85 + PR-2 86-93 + PR-3 94-95 포함)
  - 신규 1건 = lock candidate 와 정확히 일치, id = 96
  - PR-4 범위 외 신규 0 · needs_review(에티드론산/케토코나졸/카르바마제핀) 신규 0
  - published/clinical_reviewed=false · reviewed_by 부재(전건)
  - product/clinical/potassium=false (신규)
  - 신규 absorption/separation · counterpart_category=al_mg_antacid
  - source itemSeq + pointer · 제품/schedule 누출 0
  - display/management 금칙어·지시 0 · 신규 display '효과가 감소' 0 · '분리하도록 안내' 0
  - DATA_URL == v0.2
사용: python3 scripts/validate_pr4_h7f3001_post_integration_v1_4.py [--base 7f07df4]
"""
import argparse
...
LOCK = os.path.join(REPO, "data", "review", "pr4_h7f3001_candidate_lock_v1_4.json")
...
def load_vfp():
    spec = importlib.util.spec_from_file_location("fh", os.path.join(HERE, "fix_harvester_display_template_v1_6.py"))
...
def main():
    ...
    ap.add_argument("--base", default="7f07df4", help="baseline commit (통합 전 94·PR-3 머지)")
    args = ap.parse_args()
    ...
    print("=== PR-4 H7-F3-001 post-integration validator ===")
    ...
    if base is None:
        ...
        ok("baseline 94건 변경/삭제 0 (verbatim·PR-1 62-85 + PR-2 86-93 + PR-3 94-95 포함)", unchanged)
        ok("신규 id = live − baseline (정확히 1)", set(live_by_id) - set(base_rels) == new_ids,
           str(sorted(set(live_by_id) - set(base_rels))))
        ok("PR-1 62-85 + PR-2 86-93 + PR-3 94-95 전건 보존", all(i in live_by_id for i in range(62, 96)))

    expected_pairs = {(c["drug_ingredient"], c["counterpart"]): c for c in lock["candidates"]}
    new_rels = [r for r in rels if r["id"] in new_ids]
    ok("신규 relation 수 == 1", len(new_rels) == 1, str(len(new_rels)))
    got_pairs = {(r["ingredient"], r["nutrient"]) for r in new_rels}
    ok("신규 1쌍 == lock 후보쌍", got_pairs == set(expected_pairs.keys()), str(got_pairs ^ set(expected_pairs.keys())))
    ok("PR-4 범위 외 신규 relation 0", len(new_ids) == 1)
    ...
    ok("신규 product/clinical/potassium=false · reviewed_by 부재", not bad_flag, str(bad_flag))
    ...
    ok("신규 absorption/separation · al_mg_antacid", not bad_enum, str(bad_enum))
    ...
    for r in new_rels:
        ...
        if "효과가 감소" in txt:
            bad_eff.append(r["id"])
    ok("신규 display/management 금칙어·지시 0", not bad_copy, str(bad_copy[:3]))
    ok("신규 display 라벨귀속 '분리하도록 안내' 단정 비노출", not bad_attr, str(bad_attr))
    ok("신규 display/management '효과가 감소' 0 (source-additive 비노출)", not bad_eff, str(bad_eff))
    ...
    if _fail:
        ...
        return 1
    print("RESULT: PASS — PR-4 H7-F3-001 통합 후 94→95 · 범위 정확 · 보호상태 유지 · source/금칙/attribution/효과감소 OK")
    return 0
