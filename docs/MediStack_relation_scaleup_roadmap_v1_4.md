...
>
> **F4/F6/F10 small-family bundle 통합 준비(2026-06-17, live 0):** 마지막 unpackaged 3 family(각 reviewer-ready 1)를 small-family bundle 로 재검증 → **통합 가능 2건**(F4 1 + F6 1 + F10 0·60→62). 🔑 헤드라인 3: ①**케토코나졸×제산제(0275) route/availability 강등** — 소스 더마졸정 **수출용** oral tablet 이나 국내 색인 케토코나졸 10품목 전부 **외용(액/크림)** → 경구 흡수×제산제 카드 외용 제품 오부착 위험. 광역 L8(소스 tablet→pass)이 놓친 것을 family L6_route_domestic_availability(full index 형태 분류)가 fail → **needs_review**(live 선례 id61 이트라코나졸은 국내 제품). ②**레보티록신×제산제(0173) Al-only copy_change** — 라벨 '알루미늄 함유 제산제'만 명시(Mg 미명시) → display Mg 비단정·'알루미늄 함유 제산제(약물)' 한정(live id61 도 동일 일반화 surface). ③**에스오메프라졸×B12(0201) PPI 톤 정합 copy_change** — 오메프라졸(id13) S-거울상 → PPI×B12 5건+메트포르민 live 계열 합류·draft '수치 변화'→live 표준 템플릿. 선행조건 0(id61·id12/id13 렌더 선례)·index 자동 flip 0·latent 0(레보티록신 covered·에스오 미색인·케토 외용). **글로벌 family map 갱신: pending 3→0(전부 재검증)·통합 가능 31→33**. 조합 **F1+F2+F3+F9+F4+F6 60→93**. Factory v1.5 = 보류(packaging 조건 충족·live PR 병목). `scripts/integrate_f4_f6_f10_small_family_batch_v1_4.py` + docs `MediStack_*_f4_f6_f10_small_family_v1_4.md`·`MediStack_factory_v1_5_trigger_policy_v1_4.md`. ↓이전:
