#!/usr/bin/env python3
"""
validate_coverage_queue_draft_batch_v1_2.py
MediStack — coverage-queue relation factory **draft batch(CQFxx) 정합성·안전성** 검증기(읽기전용).
validate_factory_draft_batch_v1_2.py 패턴 승계, 입력만 batch2 파일로 교체.

강제:
  ①모든 draft 봉인 플래그: published=false·clinical_reviewed=false·reviewed_by 공란·source_confirmed=true·
  ...
  ②칼륨 nutrient 행 → potassium_safety_card=true ∧ product_link_allowed=false / 비-칼륨 → card=false
  ③source {type=허가사항, url itemSeq, pointer 확인일·인용, checked_at} 정합
  ④draft 카피(display/management) 금지어 0 (참고정보 톤)
  ⑤출처 후보가 실제 source_confirmed + 적대적 final_verdict=confirm 인지 교차검증(hold/reject/needs_review 미혼입)
  ⑥금지 성분(에스오메프라졸·와파린) 미유입
  ⑦라이브 export(relations 57)·published=false 무변경(이 batch 는 라이브 미반영)
  ⑧count=0(confirmed 0) 도 정상 PASS — 잘못된 relation 차단이 처리량보다 우선.

사용: python3 scripts/validate_coverage_queue_draft_batch_v1_2.py
종료 코드: 0 PASS, 1 FAIL
"""
import csv
import json
...
ADV = os.path.join(DATA, "coverage_queue_adversarial_verify_v1_2.json")
...
def main():
    ...
    for d in drafts:
        ...
        ck(f"{did} 후보 source_confirmed({cid})",
           rrow and rrow["source_status"] == "source_confirmed", cid)
        ck(f"{did} 후보 pass_to_draft=true({cid})", rrow and rrow["pass_to_draft"] == "true")
        ck(f"{did} 후보 적대적 final_verdict=confirm({cid})", vv and vv.get("final_verdict") == "confirm")
    ...
    for ok, name, detail in checks:
        if not ok:
            ...
            fails += 1
    print("=" * 64)
    print(f"coverage-queue draft batch CQF (count={len(drafts)}) | 검사 {len(checks)} | "
