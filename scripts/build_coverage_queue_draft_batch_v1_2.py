...
def main():
    ...
    for i, cid in enumerate(confirmed, start=1):
        ...
        d = {
            "draft_id": draft_id,
            "source_batch": "coverage_queue_factory_v1_2",
    ...
    meta = {
        "draft_name": "coverage_queue_draft_batch_v1_2",
        ...
        "note": ("coverage 우선순위 큐(Top100) precheck → 안전후보 nedrug source check → 적대적 검증(독립 회의론자 "
                 ...
                 "라이브 export(relations 57)에는 반영되지 않았고 어떤 것도 구현하지 않는다. 각 행 published=false·"
    ...
    print("\n라이브 미반영(live_integration_forbidden=true). relations 57 그대로.")
    return 0
