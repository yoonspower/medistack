...
> **F2 테트라사이클린 5건 통합 준비(2026-06-17):** reviewer-ready F2 5건 → reviewer-gated 통합 준비(live 0·60→65·선행조건 0·독시/미노 nutrient-overlap reviewer 판단). 정본 `docs/MediStack_reviewer_package_f2_tetracycline_v1_4.md` + `scripts/integrate_f2_tetracycline_batch_v1_4.py`. 본 백로그와 별개 트랙.
...
- theme map reviewer-gated 라운드(2026-06-16, PR #3 merge 후) 변경분 = 신규 scripts(`integrate_theme_map_draft_batch_v1_3.py` dry-run·`test_theme_map_reviewer_note_gate_v1_3.py`·`validate_theme_map_live_dryrun_v1_3.py`·`smoke_theme_map_live_dryrun_v1_3.py`) + `data/review/theme_map_live_dryrun_v1_3.json`(60→66 예상·live_write 0) + docs(reviewer package·category policy·grouping strategy·next_prompts 10~13) 만. live relation 0·export `62df9284…` 불변·schedule 비활성·src/validator/.github 무수정. guard `--run-bot` PASS.
- 페니실라민 FE/ZN subset 라운드(2026-06-16) 변경분 = 신규 scripts(`integrate_penicillamine_subset_v1_3.py` dry-run·`test_penicillamine_reviewer_note_gate_v1_3.py`·`validate_penicillamine_subset_dryrun_v1_3.py`(결함주입9)·`smoke_penicillamine_subset_v1_3.py`) + `data/review/penicillamine_subset_live_dryrun_v1_3.json`(60→62·**선행조건 0**·live_write 0) + docs(reviewer package penicillamine·mechanism decision·next_prompts 14~16) + 기존 docs 갱신 만. theme map 6건 중 FE/ZN 2건(counterpart_category=null·일반 영양소)은 validator/src 선행조건 없이 최우선 통합 가능(현행 v0.2 PASS 실증). ZN mechanism=Option A(absorption 추론). live relation 0·export `62df9284…` 불변·schedule 비활성·src/validator/full index/aliases/.github 무수정. guard `--run-bot` PASS.
- **Relation Factory Bot v1.4 라운드(2026-06-16, HEAD 2aa3a07 후)** = 1,000 relation scale-up 대량 후보 공장 구축·실행. 신규 scripts `build_relation_factory_inventory_v1_4.py`·`relation_factory_bot_v1_4.py`(family universe 11·offline 안전·`--online-source-check` SDK-only)·`validate_relation_factory_batch_v1_4.py`(결함주입 10)·`smoke_relation_factory_batch_v1_4.py` + data/review·drafts 산출(inventory·universe·candidates·queue·precheck·source-check results·draft batch·PM queue) + docs(scaleup roadmap·family universe·candidates·inventory). **raw 후보 301·신규 262·source-check queue 200·SDK source-check 200(network~27 캐시)·source_confirmed draft 43**(FQ/tetracycline/bisphosphonate×mineral·antacid·AED×folate/vitD·azole×antacid·PPI×B12). 🔑품질검수로 false positive 다수 제거: Mg 영양제 오인(제산제 Mg)·MTX/피리메타민 고위험·발프로산 엽산보충권유·록사티딘/오메프라졸 부정문·졸레드론산 IV·테트라사이클린 골복합체·항응고 quote 혼입. **live relation 0·export `62df9284` 불변·src/validator/.github/full-6 무수정·schedule 무관·제품/구매/제휴 0.** family 수확률: F9 0.36·F10 1.0·F1 0.21. no_domestic 90(미유통 fail-closed). 정본 roadmap `docs/MediStack_relation_scaleup_roadmap_v1_4.md`.
- 페니실라민 reviewer decision 하드닝 라운드(2026-06-16, HEAD 15cd8bf 후) = reviewer note 실물 **없음 → live 0** 확정. 변경분 = 통합기에 `partial_approval_scenarios`(both 60→62·FE_only/ZN_only 60→61·neither 60·**id rule: 단건 승인=id 62=max+1**) + `--only` STOP(미구현·both-approval 전제) · validator 부분승인 일관성 + 결함주입 **10종**(ZN_only id 오기 추가) · reviewer package §8 결정 체크리스트/§9 PM decision table/§10 FE-only·ZN-only 근거/§11 rollback·post-live 검증 · mechanism doc §5 Option 체크박스 · next_prompts. **full-6 중복 생성 불가 실증**(FE/ZN live 인 temp 에 full-6 build → 2 violation→STOP). live relation 0·export `62df9284…` 불변.
