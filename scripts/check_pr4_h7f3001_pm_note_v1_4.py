#!/usr/bin/env python3
"""
check_pr4_h7f3001_pm_note_v1_4.py
MediStack v1.4 — PR-4 **H7-F3-001** PM-reviewed verified-reference note **checker** (읽기전용·네트워크 0·live 무수정).

PR-4 = F3 리세드론산 × Al/Mg 함유 제산제 1건. relation_count 94 → 95.
(absorption/separation · counterpart_category=al_mg_antacid · drug counterpart.)
...
(1) PM 토큰 전건 (2) candidate 1 (3) 보호 상태 유지 승인 (4) 금지 행위 미요구
(5) source-fidelity copy 원칙(리세드론산 라벨 흡수-방해만 명시 → '효과가 감소' 비노출 · Al/Mg counterpart 정당) 임을 확인한다.

wave→candidate_ids/delta 는 data/review/pr4_h7f3001_candidate_lock_v1_4.json 에서 읽는다(단일 진실원).

사용:
  python3 scripts/check_pr4_h7f3001_pm_note_v1_4.py --note data/review/pr4_h7f3001_pm_reviewed_note_v1_4.md
PASS=종료코드 0, FAIL=1.
"""
import argparse
import json
...
LOCK = os.path.join(ROOT, "data", "review", "pr4_h7f3001_candidate_lock_v1_4.json")
...
# PR-4 in-scope = F3(H7-F3-001). F1/F2/F4/F6/F9/F10 candidate 가 승인 맥락(제외 라인 밖)에 등장하면 거부.
EXCLUDED_FAMILY_RE = re.compile(r"\bRF-(F1|F2|F4|F6|F9|F10)-\S+")
...
def check_pm_note(note_path, lock):
    ...
    if lock["total"] != 1:
        problems.append(f"PR-4 는 단건 — lock total {lock['total']} != 1")
    ...
    if "h7f3001" not in low and "H7-F3-001" not in t:
        problems.append("wave(PR-4 h7f3001) 명시 없음")
    if "live/pr4-h7f3001" not in t and "target" not in low and "branch" not in low:
    ...
    if "리세드론산" not in t:
        problems.append("scope: 리세드론산 명시 없음")

    # 7) needs_review / 제외 family / PR-1·PR-2·PR-3 재추가 금지 — '제외' 맥락에서만 허용
    nr_bad, fam_bad = [], []
    for line in t.splitlines():
        ...
        for full in EXCLUDED_FAMILY_RE.finditer(line):
            ...
            if not in_scope and "제외" not in line and "PR-1" not in line and "PR-2" not in line \
               and "PR-3" not in line and "PR1" not in line and "PR2" not in line and "PR3" not in line:
                fam_bad.append(fid)
    if nr_bad:
        problems.append(f"needs_review candidate 포함(승인 맥락): {sorted(set(nr_bad))}")
    if fam_bad:
        problems.append(f"F1/F2/F4/F6/F9/F10 candidate 포함(승인 맥락): {sorted(set(fam_bad))}")
    ...
    if ("PR-1" not in t and "PR-2" not in t and "PR-3" not in t) \
       or ("재추가" not in t and "재통합" not in t and "중복" not in t):
        problems.append("PR-1/PR-2/PR-3 후보 재추가 금지 확인 문구 없음")
    # 항생제 minor 3건(id67/71/75) 비포함 명시
    if not re.search(r"67|71|75", t) or ("minor" not in low and "copyfix" not in low and "비포함" not in t):
        problems.append("항생제 minor 3건(id67/71/75) 비포함/copyfix 별도 처리 명시 없음")
    ...
    # 9) source fidelity / copy_change / al_mg_antacid / 리세드론산 Al/Mg counterpart 정당
    if "출처" not in t or ("일치" not in t and "보존" not in t):
        problems.append("source fidelity(출처 일치/보존) 확인 없음")
    if "itemSeq" not in t or "200713889" not in t:
        problems.append("itemSeq 200713889 명시 없음")
    if "al_mg_antacid" not in t and "제산제" not in t:
        problems.append("counterpart category(al_mg_antacid/제산제) 명시 없음")
    if not re.search(r"(absorption|흡수)", t) or not re.search(r"(separation|분리|복용\s*시점)", t):
        problems.append("mechanism/action(absorption/separation·복용 시점 분리) 명시 없음")
    # 리세드론산 라벨 흡수-방해만 명시 → '효과가 감소' 비노출/제거 근거
    if "효과가 감소" not in t or ("비노출" not in t and "제거" not in t and "미언급" not in t
                              and "미명시" not in t and "없" not in t):
        problems.append("'효과가 감소' 비노출 근거(라벨 흡수만 명시) 인지 없음")
    # Al/Mg counterpart 정당(라벨 알루미늄·마그네슘 명시)
    if ("Al/Mg" not in t and "알루미늄" not in t) or ("마그네슘" not in t and "Mg" not in t):
        problems.append("Al/Mg counterpart 정당(라벨 알루미늄·마그네슘 명시) 인지 없음")
...
def main():
    ap = argparse.ArgumentParser(description="PR-4 H7-F3-001 PM-reviewed note checker (no write)")
    ...
    if ok:
        print(f"PASS — PR-4 H7-F3-001 PM note 유효 "
              f"(candidate {lock['total']} · delta +{lock['relation_delta']} · "
              f"{lock['baseline_relation_count']}→{lock['expected_relation_count_after']} · "
              f"needs_review/F1·F2·F4·F6·F9·F10 제외 · 항생제 minor 67/71/75 비포함 · PM 토큰 {len(PM_TOKENS)})")
        return 0
    print(f"FAIL — PR-4 H7-F3-001 PM note 거부 ({len(problems)}건):")
    for p in problems:
        print("  -", p)
    return 1
