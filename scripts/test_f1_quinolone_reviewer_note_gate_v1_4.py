#!/usr/bin/env python3
"""
test_f1_quinolone_reviewer_note_gate_v1_4.py
MediStack — F1 퀴놀론 live 통합 reviewer-note **인터록 회귀 테스트**(읽기전용·네트워크 0).

검증 대상: integrate_f1_quinolone_batch_v1_4.check_reviewer_note + (temp-copy) 전체 write 경로.
  1) 게이트 단위 — invalid(빈/토큰없음/candidate 일부 누락/scope 불일치/SAMPLE/placeholder/clinical=true 요구/
     제품 추천 허용/금속이온·제산제 복용 권유 허용/al_mg_antacid 미명시(mineral·antacid 혼동)/verified_reference 없음/
     reviewer 식별자 없음) 거부 + valid 통과.
  2) temp-copy 전체 write — valid 노트로 --pm-approved --reviewer-note 를 **임시 복사본** export 에 실행해
     all18 60→78 / nutrient10 60→70 / antacid8 60→68 동작 + **실제 live export sha256 불변** 확인.
...
def build_valid(mod, scope_label, scope_ids):
    ...
    grouping = {"all18": "grouping: all18 한 번에 통합", "nutrient10": "grouping: by-counterpart nutrient subset",
                "antacid8": "grouping: by-counterpart al_mg_antacid subset"}[scope_label]
    return (
        f"검수자: RPH-TEST-001 (PM 승인 근거 첨부)  검토일 2026-07-01\n"
        f"승인(approved): F1 퀴놀론 {scope_label} 후보를 verified_reference 노출로 승인.\n"
...
def test_gate(mod, tmp):
    ...
    expect_reject("grouping 결정 누락 거부",
                  cn(VALID.replace("grouping: all18 한 번에 통합", "통합 방식 미정"))[1], "grouping")
    expect_reject("al_mg_antacid 미명시(mineral/antacid 혼동) 거부",
    ...
    expect_reject("금속이온 복용 권유 아님 명시 누락 거부",
                  cn(VALID.replace(" 금속이온·제산제 복용 권유 없음.", ""))[1], "복용 권유 아님")
    ...
    expect_reject("금속이온/제산제 복용 권유 허용 거부",
                  cn(VALID.replace("금속이온·제산제 복용 권유 없음.", "철분 보충 권장함."))[1], "복용 권유/권장 허용")
    expect_accept("valid 노트(all18) 통과", cn(VALID)[1])
    # scope 불일치 — nutrient10 ids 로 all18 노트 검증(antacid ids 누락 감지)
    expect_reject("scope 불일치(all18 노트를 antacid8 scope 로) 거부",
                  mod.check_reviewer_note(_note(tmp, build_valid(mod, "nutrient10", nutrient)), antacid)[1],
                  "candidate_id 미명시")
    expect_accept("valid 노트(nutrient10) 통과",
                  mod.check_reviewer_note(_note(tmp, build_valid(mod, "nutrient10", nutrient)), nutrient)[1])
    expect_accept("valid 노트(antacid8) 통과",
                  mod.check_reviewer_note(_note(tmp, build_valid(mod, "antacid8", antacid)), antacid)[1])
...
def temp_write(mod, tmp, scope_label, expected_after, expected_ids):
    ...
    recs, _s = mod.load_f1()
    nutrient, antacid = mod._scope_split(recs)
    scope_ids = {"all18": [r["candidate_id"] for r in recs], "nutrient10": nutrient, "antacid8": antacid}[scope_label]
    work = tempfile.mkdtemp(prefix="ms_f1_write_")
    old_argv = sys.argv
    try:
...
def test_idempotency(mod, tmp):
    ...
    before = _sha(real)
    recs, _s = mod.load_f1()
    work = tempfile.mkdtemp(prefix="ms_f1_idem_")
    old_argv = sys.argv
    try:
        # nutrient10 을 미리 통합한 export 를 만든 뒤, 같은 scope 재실행 → 중복 감지 STOP.
        tmp_export = os.path.join(work, "export.json")
        shutil.copy(real, tmp_export)
        nutrient, _ = mod._scope_split(recs)
        note = _note(tmp, build_valid(mod, "nutrient10", nutrient))
        mod.EXPORT = tmp_export
        sys.argv = ["prog", "--pm-approved", "--reviewer-note", note, "--scope", "nutrient10"]
        mod.main()  # 1차 통합
        rc2 = mod.main()  # 2차 — 이미 존재 → build_subset viol → STOP(1)
        ok = rc2 == 1
        print(("  PASS " if ok else "  FAIL ") + "nutrient10 재실행 STOP(이미 live)" + ("" if ok else f" rc2={rc2}"))
        if not ok:
...
def main():
    mod = _load("integrate_f1_quinolone_batch_v1_4.py")
    tmp = tempfile.mkdtemp(prefix="ms_f1_gate_")
    try:
        print("=== F1 퀴놀론 reviewer-note 게이트 회귀 테스트 (live 무수정) ===")
        test_gate(mod, tmp)
        temp_write(mod, tmp, "all18", 78, list(range(62, 80)))
        temp_write(mod, tmp, "nutrient10", 70, list(range(62, 72)))
        temp_write(mod, tmp, "antacid8", 68, list(range(62, 70)))
    ...
    if fails:
        ...
        return 1
    print("RESULT: PASS — invalid 전건 거부 + valid 통과 · temp write all18/nutrient10/antacid8 동작 · "
